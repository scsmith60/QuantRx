...
const TheMorningList: React.FC = () => {
    ...
    useEffect(() => {
        // Mocking "The Morning List" (Today's Schedule)
            ...
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-[10px] text-muted-foreground uppercase tracking-widest mt-1">
                        Daily Patient Optimization Queue • {new Date().toLocaleDateString()}
                    </p>
                </div>
                <div className="flex items-center space-x-2 px-3 py-1 bg-primary/10 border border-primary/20 rounded-full">
                    <span className="text-[10px] font-bold text-primary tracking-tight">
                        {Object.values(optimizations).filter(o => o.type !== 'NONE').length} WINNERS DETECTED
                    </span>
                </div>
            </div>
